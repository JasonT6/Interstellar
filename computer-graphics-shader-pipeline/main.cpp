...
Eigen::Affine3f view = 
  ...
  Eigen::Translation3f(Eigen::Vector3f(0,0,-30));
...
bool galaxy_texture_loaded = false;
...
int main(int argc, char * argv[])
{
  ...
  const char* json_path = (argc > 1) ? argv[1]: "../data/interstellar.json";
  ...
  GLFWwindow* window = glfwCreateWindow(width, height, "Interstellar", NULL, NULL);
  if(!window)
  {
    glfwTerminate();
...
Usage:
  ...
  ma_sound ambience;
  if (ma_sound_init_from_file(&audio_engine, "../audio/interstellar.mp3", MA_SOUND_FLAG_STREAM, NULL, NULL, &ambience) != MA_SUCCESS) {
    std::cerr << "Failed to load audio file\n";
  }

  ma_uint32 sampleRate = ma_engine_get_sample_rate(&audio_engine);
  ma_sound_seek_to_pcm_frame(&ambience, (ma_uint64)(10.0f * sampleRate));

  ma_sound_set_looping(&ambience, MA_TRUE);
  ma_sound_set_volume(&ambience, 0.6f);

  ma_sound_start(&ambience);
  
  int w, h, n;
  stbi_set_flip_vertically_on_load(true);

  float* data = stbi_loadf("../images/HDR_rich_multi_nebulae_1.hdr", &w, &h, &n, 0);
  if (!data) {
    std::cerr << "Failed to load HDR_rich_multi_nebulae_1.hdr\n";
    exit(1);
  }

  glGenTextures(1, &galaxy_texture);
  ...
  glfwSetScrollCallback(window,
    ...
    {
      view.matrix()(2,3) =
        std::min(std::max(view.matrix()(2,3)+(float)yoffset,-55.0f),-10.0f);
    });
  ...
  view.rotate(Eigen::AngleAxisf(0.5f, Eigen::Vector3f(0,-1,0)));
  while (!glfwWindowShouldClose(window))
  {
    ...
    glBindTexture(GL_TEXTURE_2D, galaxy_texture);
    glUniform1i(glGetUniformLocation(prog_id, "skyTex"), 0);
    // Attach uniforms
    {
      if(is_animating)
      {
        ...
        last_time = now;
        view.rotate(Eigen::AngleAxisf(0.00035f, Eigen::Vector3f(0,1,0)));
      }
    ...
    //   glBindVertexArray(0);
    // }
    glUniform1i(glGetUniformLocation(prog_id, "render_grid"), show_grid);
    glBindVertexArray(VAO);
    glDrawElements(GL_PATCHES, F.size(), GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
    ...
    // 60 fps
    {
      ...
      const double min_duration = 1000000./120.;
